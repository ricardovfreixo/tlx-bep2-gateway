        ...
        for transaction in blockToCheck['transactions']:
            if transaction['type'] == 4 and transaction['recipient'] == self.config['tn']['gatewayAddress'] and transaction['assetId'] == self.config['tn']['assetId']:
                ...
                if len(targetAddress) > 1 and self.txNotYetExecuted(transaction['id'], dbCon):
                    ...
                    cursor.execute('INSERT INTO executed ("sourceAddress", "targetAddress", "tnTxId", "ethTxId", "timestamp", "amount", "amountFee") VALUES ("' + transaction['sender'] + '", "' + targetAddress + '", "' + transaction['id'] + '", "outgoing not supported", "' + timestampStr +  '", "' + str(amount) + '", "' + str(self.config['erc20']['fee']) + '")')
                    dbCon.commit()
                    print ('unallowed transaction detected, check executed table for details')
